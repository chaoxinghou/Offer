...
public class DoubleLock implements Serializable {
    ...
    private static final DoubleLock instance = new DoubleLock();
    ...
    public static DoubleLock getInstance2() {
        //第一次判断，假设会有好多线程，如果doubleLock没有被实例化，那么就会到下一步获取锁，只有一个能获取到，
        //如果已经实例化，那么直接返回了，减少除了初始化时之外的所有锁获取等待过程
        if (doubleLock == null) {
            doubleLock = new DoubleLock();
    ...
    public static DoubleLock getInstance3() {
        return instance;
    }
